...
X(:,inan)=[];
 ...
 AMMpre = zscore(SCORE(:,1));
 
  NP = polyfit(AMMpre(1:39,:),AMM_Ajs(:,2),2);
  NPfit = polyval(NP,AMMpre);
  corr(NPfit(1:39,:),AMM_Ajs(:,2))
...
subplot(3,1,1)
    ...
    hold on 
    %x1=0;   y1=-0.56; x2=42;   y2=-0.56; X=[x1 x2]; Y=[y1 y2]; line(X,Y,'Color','red','LineStyle','--','LineWidth',1)
    ...
    %text(42,-0.7,'Seco','Color','red','Fontsize',12)
    xlim([0 44])
    ylim([-4 4])
    grid on
    title('GLM: Pronóstico PC1 - SPI para EFM Sierra Sur (ic Ene)','FontSize',19,'FontWeight','bold')
    legend('PC1-SP1','CFSv2', 'CanCM4I','GEN NEMO','CANSIPv2')
    %legend('PC1-SP1', 'CanCM4I','GEN NEMO','CANSIPv2')
...
subplot(3,1,2)
    ...
    ylim([-4 4])
    grid on
    title('GLM: Pronóstico PC2 - SPI para EFM Sierra Sur (ic Ene)','FontSize',19,'FontWeight','bold')
    %legend('PC2-SP1','CFSv2', 'CanCM4I','GEN NEMO','CANSIPv2')
    %legend('PC1-SP1','CanCM4I','GEN NEMO','CANSIPv2')
...
subplot(3,1,3)
    ...
    ylim([-4 4])
    grid on
    title('GLM: Pronóstico PC3 - SPI para EFM Sierra Sur (ic Ene)','FontSize',19,'FontWeight','bold')
    %legend('PC2-SP1','CFSv2', 'CanCM4I','GEN NEMO','CANSIPv2')
    %legend('PC1-SP1','CanCM4I','GEN NEMO','CANSIPv2')
...
%PCA
%%%%% Modos de la matriz de correlación 
...
lon=double(ncread('Referencia.nc','lon'));
lat=double(ncread('Referencia.nc','lat'));
